...
class View:
		...
		for obj in self.objects:
			...
			if obj.dies and obj.dead:
				...
				if obj.parents == None:
					...
					if self.check_parents(obj, rooms):
						if obj.examine and obj.activated:
							examining = [obj.image, obj.rect_on]
						else:
						#screen.blit(obj.image, (obj.x, obj.y))
	...
	def click_check(self, mpos, rooms):
	...
	def check_parents(self, obj, rooms):
		validates = True
		if obj.parents == None and obj.antiparents == None:
			return True
		if obj.parents != None:
			for parent in obj.parents:
				# get the room/view name so we can check activation status
				# of objects in any room/view
				props = parent.split('_')
				room = props[0]
				view = props[1]
				if not(rooms[room][view]['view'].objects[parent].activated):
		...
		if obj.antiparents != None:
			...
			for antiparent in obj.antiparents:
				# get the room/view name so we can check activation status
				# of objects in any room/view
				props = antiparent.split('_')
				room = props[0]
				view = props[1]
				if rooms[room][view]['view'].objects[antiparent].activated:
					validates = False
		return validates
